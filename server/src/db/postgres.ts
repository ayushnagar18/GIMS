...
client.query(
  "CREATE TABLE IF NOT EXISTS services(id UUID NOT NULL PRIMARY KEY,servicetype TEXT NOT NULL,imglocation TEXT NOT NULL , name TEXT NOT NULL,price TEXT NOT NULL)",
  (err) => {
    if (err) {
      console.log(err.stack);
    }
...
client.query(
  ...
  (err) => {
    if (err) {
      ...
      console.log("Added 'price' column to services table");
    }
  }
);
...
client.query(
  "CREATE TABLE IF NOT EXISTS requirements(id UUID NOT NULL PRIMARY KEY,name TEXT,designation TEXT,email TEXT,companyname TEXT,mobile TEXT,address TEXT,fieldofservice TEXT,requirements TEXT,hours TEXT,price TEXT,filelocation TEXT,date DATE,serviceid UUID NULL REFERENCES services(id) ON DELETE CASCADE ON UPDATE CASCADE,completedby TEXT NULL REFERENCES usertable(userId),completedOn timestamptz NULL,status TEXT);",
  (err) => {
    if (err) {
      console.log(err.stack);
    }
